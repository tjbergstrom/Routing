...
func new_prefix_tree() *PrefixTree {
...
func (p *PrefixTree) Add(r Route) {
	var past_byte_bits, i, j uint8
	current := p
	for i = 0; i < 4; i++ {
		past_byte_bits = i * 8
		for j = 0; j < 8; j++ {
			if r.Prefix == (j + past_byte_bits) {
			...
			if 1 == 1&(r.Masked[i]>>(7-j)) {
				// Step deeper as 1
				if current.One == nil {
					current.One = new_prefix_tree()
				}
				current = current.One
			} else {
				// Step deeper as 0
				if current.Zero == nil {
					current.Zero = new_prefix_tree()
				}
...
// Get retrieves a route from the table, returning a pointer to the destination if found
