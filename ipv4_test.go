...
func test_mask_from_prefix(t *testing.T) {
	cases := []struct {
		Name          string
		Input         uint8
		Expected      IPv4
	...
	for _, test := range cases {
		t.Run(test.Name, func(t *testing.T) {
			got, err := mask_from_prefix(test.Input)
			if err != nil {
...
func test_mask_with(t *testing.T) {
	cases := []struct {
		Name     string
		Address  IPv4
		Mask     IPv4
	...
	for _, test := range cases {
		t.Run(test.Name, func(t *testing.T) {
			masked := test.Address.mask_with(test.Mask)
